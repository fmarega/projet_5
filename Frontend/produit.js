...
document.getElementById('ajoutPanier').addEventListener('click', () => {  //reagir lors d'un clic sur le button (ajouter au panier)
    let app = {  //obj ours pour la création panier (+localStorage)
        id: response._id,
        name: response.name,
        choix: colors.value,  //pour récupérer seulement la couleur choisie
        price: response.price,
        qte: parseInt(document.getElementById("qte").value), //analyse l'element fourni en argument et renvoie un entier exprimé dans une base donnée.
        image: response.imageUrl
    }

    const itemAdd = localStorage.getItem('product'); //la fonction pour la lecture de l'article localStorage dans le tableau

    if (itemAdd) { //Si le panier contient déja un produit, il va ajouter le nouveu produit à la fin du tableau
